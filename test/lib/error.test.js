...
describe('.__error()', function () {
	...
	it('should use default type/message when called with no arguments', function() {
		...
		expect(console.log.getCall(1)).to.have.been.calledWithExactly('Error in Agentia Ping Handler Middleware\n');
		expect(console.log.getCall(2)).to.have.been.calledWithMatch(/An unexpected error has occured/);
		expect(console.log.getCall(2)).to.have.been.calledWithMatch(/PingHandlerError/);
		expect(console.log.getCall(3)).to.have.been.calledWith('See https://github.com/JohnnyEstilles/agentia-ping-handler');
